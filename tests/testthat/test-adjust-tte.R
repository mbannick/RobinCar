...
test_that("data preprocessing", {
  ...
  # CSL for LOGRANK ----------------------------------------
  # Should have a car_strata with multiple strata values
  ...
  expect_true(length(unique(df_csl$strata)) != 1)
  expect_true(is.factor(df_csl$strata))
  ...
  expect_true(length(unique(df_cl$strata)) == 1)
  ...
  expect_true(length(unique(df_score$strata)) == 1)
  ...
  expect_true(length(unique(df_score1$strata)) == 1)
