...
#'@export
data_gen<-function(n,theta,randomization,p_trt,case=c("case1","case2","case3","case4","case5")){
  if(case=="case1"){ # multiple z
    ...
    t<-pmin(t.star,C)
    delta<-1*(t.star<=C)
    data.simu<-data.frame(t, delta, I, 1-I,z1[1,],z2[1,],z2[2,],strata_z,minimization_z)[1:n,]
    names(data.simu)<- c("t", "delta", "I1", "I0","model_Z1","model_Z21","model_Z22",paste0("strata",1:n_strata),paste0("minimization",1:5))
    return(data.simu)
  }
  else if (case=="case2"){ # coutinuous z
    z1<-rmultinom(n,1,c(0.5,0.5))
    ...
    t<-pmin(t.star,C)
    delta<-1*(t.star<=C)
    data.simu<-data.frame(t, delta, I, 1-I,z1[1,],z2_continous^2,strata_z,minimization_z)[1:n,]
    names(data.simu)<- c("t", "delta", "I1", "I0",paste0("model_z",1:2),paste0("strata",1:n_strata),paste("minimization",1:minimization_n_col))
    return(data.simu)
  }
  else if (case=="case3"){ # square (mis)
    z1<-rmultinom(n,1,c(0.5,0.5))
    row.names(z1)<-paste0("z1_",1:2)
    n_category<-4
    tmp<-discretize_z(n,0,1,n_category)
    ...
    t<-pmin(t.star,C)
    delta<-1*(t.star<=C)
    data.simu<-data.frame(t, delta, I, 1-I,z1[1,],z2_continous,strata_z,minimization_z)[1:n,]
    names(data.simu)<- c("t", "delta", "I1", "I0",paste0("model_z",1:2),paste0("strata",1:n_strata),paste("minimization",1:minimization_n_col))
    return(data.simu)
  }
  else if (case=="case4"){ # non-PH (mis), C unif
    n_category<-4
    tmp<-discretize_z(n,0,1,n_category)
    z1_continuous<-tmp$Z
    z1<-t(tmp$Z_dis)
    ...
    t<-pmin(t.star,C)
    delta<-1*(t.star<=C)
    data.simu<-data.frame(t, delta, I, 1-I,z1_continuous,strata_z,minimization_z)[1:n,]
    names(data.simu)<- c("t", "delta", "I1", "I0","model_Z1",paste0("strata",1:n_strata),paste0("minimization",1:minimization_n_col))
    return(data.simu)
  }
  else if (case=="case5"){# non-PH (mis), C depends on I
    n_category<-4
    tmp<-discretize_z(n,0,1,n_category)
    z1_continuous<-tmp$Z
    z1<-t(tmp$Z_dis)
...
# data.simu=data_gen(100,0,"permuted_block",0.5,case="case3")
...
#'@export
data_gen2<-function(n,theta,randomization,p_trt,case=c("case1","case2","case3","case4"),blocksize=4){
  if (case=="case1"){ # Cox failure time and uniform censoring
    ...
    I<-treatment_assignment(n,strata_z,minimization_z,randomization,p_trt,blocksize)
    lambda0<-log(2)
    cumuhazard<-rexp(n)
    ...
    names(data.simu)<- c("t", "delta", "I1", "I0","model_w3",paste0("strata",1:n_strata))
    return(data.simu)
  }else if (case=="case2"){ # Cox failure time and censoring depends on I
    tmp<-discretize_z(n,0,1,n_category=2)
    w1<-tmp$Z
    z1<-t(tmp$Z_dis)
    tmp<-discretize_z(n,0,1,n_category=3)
    ...
    names(data.simu)<- c("t", "delta", "I1", "I0","model_w3",paste0("strata",1:n_strata))
    return(data.simu)
  }else if (case=="case3"){ # Non-Cox failure time and uniform censoring
    tmp<-discretize_z(n,0,1,n_category=2)
    w1<-tmp$Z
    z1<-t(tmp$Z_dis)
    tmp<-discretize_z(n,0,1,n_category=3)
    ...
    names(data.simu)<- c("t", "delta", "I1", "I0","model_w3",paste0("strata",1:n_strata))
    return(data.simu)
  }else if (case=="case4"){ # Non-Cox failure time and censoring depends on I
    tmp<-discretize_z(n,0,1,n_category=2)
    w1<-tmp$Z
    z1<-t(tmp$Z_dis)
    tmp<-discretize_z(n,0,1,n_category=3)
    ...
    names(data.simu)<- c("t", "delta", "I1", "I0","model_w3",paste0("strata",1:n_strata))
    return(data.simu)
  }else if (case=="case5"){ # Non-Cox failure time and censoring depends on I
    tmp<-discretize_z(n,0,1,n_category=2)
    w1<-tmp$Z
    z1<-t(tmp$Z_dis)
    z1.f<-as.factor(z1[1,]+2*z1[2,])
    ...
    names(data.simu)<- c("t", "delta", "I1", "I0","model_w1",paste0("strata",1:n_strata))
    return(data.simu)
  }else if (case=="case4_test"){ # Non-Cox failure time and censoring depends on I
    tmp<-discretize_z(n,0,1,n_category=2)
    w1<-tmp$Z
    z1<-t(tmp$Z_dis)
    tmp<-discretize_z(n,0,1,n_category=3)
    ...
    I<-treatment_assignment(n,strata_z,data.frame(as.factor(fz)),randomization,p_trt,blocksize)
    t.star<-exp(I*theta+0.5*w1+0.5*w2+0.5*w3)+rexp(n,1)
    C<-3-3*I+rexp(n,2)
    t<-pmin(t.star,C)
    delta<-1*(t.star<=C)
...
treatment_assignment<-function(n,strata_z,minimization_z,randomization,p_trt,blocksize=4){
  if(randomization=="SR"){
    I<-SR(n,p_trt)
